...
```{r}
t.test(tlen ~ superv, data = gorriones)
```
```{r}
t.test(eala ~ superv, data = gorriones)
```
```{r}
t.test(picolen ~ superv, data = gorriones)
```
```{r}
t.test(humlen ~ superv, data = gorriones)
```
```{r}
...
<!--- Creo que esto de a continuación está mal --->
### Comparación de covarianzas
```{r}
nX = nrow(gorrionesS)
nY = nrow(gorrionesN)
nT = nrow(gorriones)
covX = cov(select_if(gorrionesS, is.numeric))
covY = cov(select_if(gorrionesN, is.numeric))
covT = ((nX*covX)+(nY*covY))/(nX+nY)
